...
try {
  for (const childName of [".candidate", "..candidate"]) {
    const artifactChild = run([
      "create",
      ...sharedOptions.filter((option) => !option.startsWith("--evidence-directory=")),
      `--evidence-directory=${path.join(artifactDirectory, childName)}`,
      "--created-at=2026-08-06T10:00:00.000Z",
      "--expires-at=2026-08-13T10:00:00.000Z"
    ]);
    assert.notEqual(artifactChild.status, 0);
