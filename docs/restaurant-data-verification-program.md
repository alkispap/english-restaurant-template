...
Checkpoint after completing the first published missing-opening-hours batch:

| Measure | Count | Meaning |
| --- | ---: | --- |
| Canonical location records | 3,186 | Total dataset grain |
| Fresh editor-verified records | 17 | Complete current core-field checks with retained evidence |
| Unverified records | 3,169 | Historically traceable but not fully checked for current facts |
| Retained records with operational gaps | 607 | Unique canonical records missing at least one important data group |
| Ordinary queued records with operational gaps | 592 | Excludes excluded records and fresh verified records from the ordinary unverified queue |
...
| Missing opening hours | 126 | No structured current hours |
| Missing categories | 490 | No supported cuisine/category value |
...
| Published missing opening hours | 96 | Next public operational-gap cohort |
...
These figures do not mean that 3,169 restaurants are wrong. They mean the directory lacks a dated, complete, evidence-backed current check for them.
...
### V4 — 490 missing-category records
...
2. Confirm exact restaurant operation, cuisine scope and restaurant-specific hours before applying venue, event-space, hotel, pub, caterer or market hours.
3. Keep the 25 current `pending-review` records in the explicit conflict queue and revisit them only when stronger evidence is available; `chaiwrap` now belongs to this queue because current exact-premises and claimed-profile evidence conflict.
